...
# 这套 flag 是实测过能过 CF/Vercel 验证的最小集：
# --disable-gpu + --disable-software-rasterizer 会连 SwiftShader 软渲染兜底一起堵死，WebGL 直接报 no context，秒被识别成裸奔虚拟机
# --enable-unsafe-swiftshader 才是正确姿势：放行 SwiftShader，WebGL Vendor/Renderer 能吐出真实的 ANGLE/SwiftShader 信息
# --test-type 与本次要修的问题无关，之前怀疑过是它导致 chrome.runtime 异常，后来用干净对照实测证伪了，不需要它，去掉
# "不受支持的命令行标记" 黄条改用下面的 Chrome policy 文件关闭，不依赖 --test-type
